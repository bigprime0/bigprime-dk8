  ...
  <div class="container-list">
    <Breadcrumb
      :items="[
        ...
        'menu.kubernetes.config.horizontalpodautoscaler'
      ]"
    ...
    <div class="contain">
      ...
      >
        ...
        <tiny-grid-column
          field="metrics"
          :title="$t('kubernetes.hpa.metrics')"
          width="150"
          align="center"
        ></tiny-grid-column>
        ...
        <tiny-grid-column
          field="minReplicas"
          :title="$t('kubernetes.hpa.minReplicas')"
          width="150"
          align="center"
        ></tiny-grid-column>
        ...
        <tiny-grid-column
          field="maxReplicas"
          :title="$t('kubernetes.hpa.maxReplicas')"
          width="150"
          align="center"
        ></tiny-grid-column>
        ...
        <tiny-grid-column
          ...
          :title="$t('kubernetes.hpa.currentReplicas')"
          width="150"
          align="center"
        ></tiny-grid-column>

        <tiny-grid-column field="status" :title="$t('kubernetes.hpa.status')" width="120">
          <template #default="data">
            <div v-for="item in data.row.status" :key="item.type">
              <tiny-tooltip :content="item.message">
                <tiny-tag v-if="item.status == 'True'" size="small" type="success"
                  >{{ item.type }}
                </tiny-tag>
                <tiny-tag v-else size="small" type="warning">{{ item.type }}</tiny-tag>
              </tiny-tooltip>
            </div>
          </template>
        </tiny-grid-column>
...
const resourceType = ref('HPA')
const yaml = ref('')
const handleCreate = () => {
  showCreate.value = true
  yaml.value = `apiVersion: autoscaling/v2
...
const loadDatas = async () => {
  try {
    loading.value = true
    const responseString = await Kubernetes.ConfigService.listHorizontalPodAutoscalerV2(
      selectNs.value
    )
    if (responseString) {
      ...
      if (response.items.length > 0) {
        response.items.forEach((item: any) => {
          ...
          if (item.spec.metrics) {
            item.spec.metrics.forEach((item: any) => {
              Object.keys(item).forEach((key: any) => {
                if (key == 'resource') {
                  const resource = item[key]
                  if (resource.name == 'cpu') {
                    specCpu = resource.target.averageUtilization
                  }
                }
              })
            })
          ...
          let currentCpu = 0
          if (item.status.currentMetrics) {
            item.status.currentMetrics.forEach((item: any) => {
              Object.keys(item).forEach((key: any) => {
                if (key == 'resource') {
                  const resource = item[key]
                  if (resource.name == 'cpu') {
                    currentCpu = resource.current.averageUtilization
                  }
                }
              })
            })
          ...
          if (specCpu > 0) {
            field.metrics = currentCpu + '% / ' + specCpu + '%'
          }
          field.minReplicas = item.spec.minReplicas
          field.maxReplicas = item.spec.maxReplicas
          field.currentReplicas = item.status.currentReplicas
          field.status = item.status.conditions
