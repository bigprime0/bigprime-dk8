  ...
  <div class="container-list">
    <Breadcrumb
      :items="[
        ...
        'menu.kubernetes.config.poddisruptionbudget'
      ]"
    ...
    <div class="contain">
      ...
      >
        ...
        <tiny-grid-column
          ...
          :title="$t('kubernetes.pdb.maxUnavailable')"
          width="150"
          align="center"
        ></tiny-grid-column>

        <tiny-grid-column
          field="currentHealthy"
          :title="$t('kubernetes.pdb.currentHealthy')"
          width="150"
          align="center"
        ></tiny-grid-column>

        <tiny-grid-column
          field="desiredHealthy"
          :title="$t('kubernetes.pdb.desiredHealthy')"
          width="150"
          align="center"
        ></tiny-grid-column>

        <tiny-grid-column field="status" :title="$t('kubernetes.hpa.status')" width="150">
...
const loadDatas = async () => {
  try {
    loading.value = true
    const responseString = await Kubernetes.ConfigService.listPodDisruptionBudget(selectNs.value)
    if (responseString) {
      ...
      if (response.items.length > 0) {
        response.items.forEach((item: any) => {
          ...
          field.maxUnavailable = item.spec.maxUnavailable
          field.currentHealthy = item.status.currentHealthy
          field.desiredHealthy = item.status.desiredHealthy
