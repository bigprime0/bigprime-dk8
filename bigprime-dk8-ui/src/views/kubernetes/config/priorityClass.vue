<template>
  <div class="container-list">
    <Breadcrumb
      :items="['menu.kubernetes', 'menu.kubernetes.config', 'menu.kubernetes.config.priorityclass']"
    ...
    <div class="contain">
      ...
      >
        ...
        <tiny-grid-column
          field="value"
          :title="$t('kubernetes.priorityClass.value')"
          width="120"
          align="center"
        ></tiny-grid-column>
        ...
        <tiny-grid-column
          ...
          :title="$t('kubernetes.priorityClass.preemptionPolicy')"
          width="180"
          align="center"
        ></tiny-grid-column>
        ...
        <tiny-grid-column
          field="description"
          :title="$t('kubernetes.priorityClass.description')"
        ></tiny-grid-column>
...
const loadDatas = async () => {
  try {
    loading.value = true
    const responseString = await Kubernetes.ConfigService.listPriorityClass()
    if (responseString) {
      ...
      if (response.items.length > 0) {
        response.items.forEach((item: any) => {
          ...
          field.preemptionPolicy = item.preemptionPolicy
