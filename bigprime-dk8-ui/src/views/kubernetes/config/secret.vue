<template>
  <div class="container-list">
    <Breadcrumb
      :items="['menu.kubernetes', 'menu.kubernetes.config', 'menu.kubernetes.config.secret']"
    ...
    <div class="contain">
      ...
      >
        ...
        <tiny-grid-column field="labels" :title="$t('kubernetes.labels')" width="300">
          <template #default="data">
            <div v-for="(value, key) in data.row.labels" :key="key">
        ...
        <tiny-grid-column
          field="type"
          :title="$t('kubernetes.secret.type')"
          width="100"
        ></tiny-grid-column>
...
const loadDatas = async () => {
  try {
    loading.value = true
    const responseString = await Kubernetes.ConfigService.listSecret(selectNs.value)
    if (responseString) {
      ...
      if (response.items.length > 0) {
        response.items.forEach((item: any) => {
          ...
          field.labels = item.metadata.labels
