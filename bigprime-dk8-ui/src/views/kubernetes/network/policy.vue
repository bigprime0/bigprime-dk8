<template>
  <div class="container-list">
    <Breadcrumb
      :items="['menu.kubernetes', 'menu.kubernetes.network', 'menu.kubernetes.network.policy']"
    ...
    <div class="contain">
      ...
      >
        ...
        <tiny-grid-column
          field="policyTypes"
          :title="$t('kubernetes.ingress.policyTypes')"
          width="256"
        >
          <template #default="data">
            <div v-for="(key, value) in data.row.policyTypes" :key="key">
...
import ShowConfigEditor from '@/views/kubernetes/components/ShowConfigEditor.vue'
...
const loadDatas = async () => {
  try {
    loading.value = true
    const responseString = await Kubernetes.NetworkService.listPolicy(selectNs.value)
    if (responseString) {
