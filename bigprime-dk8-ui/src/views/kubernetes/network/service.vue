<template>
  <div class="container-list">
    <Breadcrumb
      :items="['menu.kubernetes', 'menu.kubernetes.network', 'menu.kubernetes.network.service']"
    ...
    <div class="contain">
      ...
      >
        ...
        <tiny-grid-column
          field="type"
          :title="$t('kubernetes.service.type')"
          width="120"
        ></tiny-grid-column>

        <tiny-grid-column
          field="clusterIP"
          :title="$t('kubernetes.service.clusterIP')"
          width="120"
        ></tiny-grid-column>

        <tiny-grid-column field="ports" :title="$t('kubernetes.service.ports')" width="150">
          <template #default="data">
            <div v-for="item in data.row.ports" :key="item.name">
              <tiny-tag
                size="small"
                type="info"
                v-if="item.nodePort != null && item.nodePort != '' && item.nodePort != undefined"
                >{{ item.port }}:{{ item.nodePort }}/{{ item.appProtocol || item.protocol }}
              </tiny-tag>
              <tiny-tag size="small" type="info" v-else
                >{{ item.port }}/{{ item.appPortocol || item.protocol }}
              </tiny-tag>
            </div>
          </template>
        </tiny-grid-column>

        <tiny-grid-column field="selector" :title="$t('kubernetes.service.selector')" width="300">
          <template #default="data">
            <div v-for="(key, value) in data.row.selector" :key="key">
              <tiny-tag size="small" type="info" style="min-width: 256px"
                >{{ key }}:{{ value }}
              </tiny-tag>
            </div>
          </template>
...
const resourceType = ref('Service')
const yaml = ref('')
const handleCreate = () => {
...
const loadDatas = async () => {
  try {
    loading.value = true
    const responseString = await Kubernetes.NetworkService.listService(selectNs.value)
    if (responseString) {
      ...
      if (response.items.length > 0) {
        response.items.forEach((item: any) => {
          ...
          field.clusterIP = item.spec.clusterIP
          field.ports = item.spec.ports
          field.selector = item.spec.selector
