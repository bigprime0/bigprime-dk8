<template>
  <div class="container-list">
    <Breadcrumb
      :items="['menu.kubernetes', 'menu.kubernetes.storage', 'menu.kubernetes.storage.pvc']"
    ...
    <div class="contain">
      ...
      >
        ...
        <tiny-grid-column field="pods" :title="$t('kubernetes.storage.pods')"></tiny-grid-column>
...
const loadDatas = async () => {
  try {
    loading.value = true
    const responseString = await Kubernetes.StorageService.listPersistentVolumeClaim(selectNs.value)
    const pods = await Kubernetes.StorageService.getPvcUsdPods(selectNs.value)
    if (responseString) {
      ...
      if (response.items.length > 0) {
        response.items.forEach((item: any) => {
          let field = {} as any
          let pvcName = item.metadata.name
          field.name = item.metadata.name
          field.namespace = item.metadata.namespace
          field.storageClass = item.spec.storageClassName
          field.size = (1.0 * item.status.capacity.storage.number) / 1024 / 1024 / 1024 + 'Gi'
          if (pods) {
            if (Object.keys(pods).includes(pvcName)) {
              field.pods = pods[pvcName]
            }
          }
