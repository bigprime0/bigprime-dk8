<template>
  <div class="container-list">
    <Breadcrumb
      :items="[
        'menu.kubernetes',
        'menu.kubernetes.storage',
        'menu.kubernetes.storage.storageclass'
      ]"
    ...
    <div class="contain">
      ...
      >
        ...
        <tiny-grid-column
          field="provisioner"
          :title="$t('kubernetes.storage.provisioner')"
        ></tiny-grid-column>

        <tiny-grid-column
          field="reclaimPolicy"
          :title="$t('kubernetes.storage.reclaimPolicy')"
          width="180"
          align="center"
        ></tiny-grid-column>
        ...
        <tiny-grid-column
          field="default"
          :title="$t('kubernetes.storage.default')"
          width="150"
        ></tiny-grid-column>
...
const resourceType = ref('StorageClass')
const yaml = ref('')
const handleCreate = () => {
  showCreate.value = true
  yaml.value = `apiVersion: storage.k8s.io/v1
kind: StorageClass
metadata:
  name: standard
...
const loadDatas = async () => {
  try {
    loading.value = true
    const responseString = await Kubernetes.StorageService.listStorageClass()
    if (responseString) {
      ...
      if (response.items.length > 0) {
        response.items.forEach((item: any) => {
          ...
          field.reclaimPolicy = item.reclaimPolicy
