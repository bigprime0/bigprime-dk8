<template>
  <div class="container-list">
    <Breadcrumb
      :items="['menu.kubernetes', 'menu.kubernetes.workloads', 'menu.kubernetes.workloads.jobs']"
...
const handleCreate = () => {
  ...
  yaml.value = `apiVersion: batch/v1
kind: Job
metadata:
  name: pi
spec:
...
const loadDatas = async () => {
  try {
    loading.value = true
    const responseString = await Kubernetes.WorkloadService.listJob(selectNs.value)
    if (responseString) {
