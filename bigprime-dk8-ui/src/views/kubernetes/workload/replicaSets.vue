  ...
  <div class="container-list">
    <Breadcrumb
      :items="[
        ...
        'menu.kubernetes.workloads.replicasets'
      ]"
...
const resourceType = ref('ReplicaSet')
const yaml = ref('')
const handleCreate = () => {
...
const loadDatas = async () => {
  try {
    loading.value = true
    const responseString = await Kubernetes.WorkloadService.listReplicaSet(selectNs.value)
    const statusData = await Kubernetes.WorkloadService.getReplicaSetsStatus(selectNs.value)
    if (responseString) {
      ...
      if (response.items.length > 0) {
        response.items.forEach((item) => {
          ...
          field.desired = item.metadata.desired
          field.current = 0
          field.ready = 0
          if (statusData && statusData.length > 0) {
            const find = statusData.find((m) => m.name == replicaSetNme)
            if (find) {
              field.desired = find.desired
              field.current = find.current
              field.ready = find.ready
            }
          }
